...
class Extractor(object):
    ...
    def __init__(self, id, title, page):
        ...
        self.frame = []
        self.wordLemma = WordNetLemmatizer()

    def lemmatizeWords(self, content):
        tokens = word_tokenize(content.decode('utf-8'))
        newContent = ""
        for each_token in tokens:
            if each_token.lower() in stopWords:
                continue
            lemmaWord = self.wordLemma.lemmatize(each_token.lower())
            newContent += lemmaWord + " "
        return newContent.rstrip().encode('utf-8')


    def extract(self, pageIDCount, out=sys.stdout):
        ...
        pageContentDict[pageIDCount] = ''
        pageLinkDict[pageIDCount] = set()
        pageIdDict[pageIDCount] =  self.title
        pageTitleToIDDict[self.title] = pageIDCount
        ...
        for line in compact(text):
            content1 += line.encode('utf-8') + " "
        soup = BeautifulSoup(content1)
        textTitle = soup.find('text')
        textLinks = soup.findAll('a')
        if (not textTitle == None):
            textContent = textTitle.get_text().encode('utf-8')
            pageContentDict[pageIDCount] = self.lemmatizeWords(textContent)
        for each_link in textLinks:
            if not (each_link.get('href') == None):
                pageLinkDict[pageIDCount].add(urllib.unquote(each_link.get('href')))
        #out.write(footer)
...
def convertDictionary():
    pageList = set(pageTitleToIDDict.keys())
    for each_entry in pageLinkDict:
        linkSet = pageLinkDict[each_entry]
        existing_links = linkSet & pageList
        newLinkID = set()
        for eachlink in existing_links:
            newLinkID.add(pageTitleToIDDict[eachlink])
        pageLinkDict[each_entry] = newLinkID

def writeToCSV():
    transitionMatrix = dict()
    with open('pageLinks.csv', 'w') as csvfile:
        for each_entry in pageLinkDict:
            linewriter = csv.writer(csvfile, delimiter=' ')
            lineList = []
            lineList.append(each_entry)
            linkSet = pageLinkDict[each_entry]
            num_links = len(linkSet)
            for each_link in linkSet:
                lineList.append(each_link)
                transitionMatrix[(each_link, each_entry)] = 1/float(num_links)
            linewriter.writerow(lineList)

    with open('pageID.csv', 'w') as csvfile:
        for each_entry in pageIdDict:
            linewriter = csv.writer(csvfile, delimiter=' ')
            linewriter.writerow([each_entry, pageIdDict[each_entry].encode('utf-8')])

    with open('pageContent.csv', 'w') as csvfile:
        for each_entry in pageContentDict:
            linewriter = csv.writer(csvfile, delimiter='\t')
            linewriter.writerow([each_entry, pageContentDict[each_entry]])


    with open('transitionMatrix.txt', 'w') as csvfile:
        for each_entry in transitionMatrix:
            linewriter = csv.writer(csvfile, delimiter=' ')
            linewriter.writerow([each_entry[0], each_entry[1], transitionMatrix[each_entry]])

    num_pages = len(pageIdDict)
    with open('vector.txt', 'w') as csvfile:
        for each_entry in pageIdDict:
            linewriter = csv.writer(csvfile, delimiter=' ')
            linewriter.writerow([each_entry, 1/float(num_pages)])
...
def main():
    ...
    groupS.add_argument("-q", "--quiet", action="store_true", default="",
    ...
    if args.article:
        if args.templates:
            if os.path.exists(args.templates):
                with open(args.templates) as file:
                    load_templates(file)
        listFiles = glob.glob("small_pages/*.xml")
        #sys.stdout.write(xmlHeader)
        #sys.stdout.write(xmlRootBeginTag)
        pageIDCount = 0
        for each_file in listFiles:
            ...
            with open(each_file) as file:
                ...
                if m:
                    title = m.group(1)
                Extractor(id, title, [page]).extract(pageIDCount)
            pageIDCount += 1
